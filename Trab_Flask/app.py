...
def criar_banco_de_dados():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS funcionarios (
            ...
            status_funcionario BOOLEAN NOT NULL DEFAULT 1,
            id_setor INTEGER,
            id_cargo INTEGER,
            FOREIGN KEY (id_setor) REFERENCES setor(id),
    ...
    with sqlite3.connect('funcionarios.db') as conn:
        ...
        funcionarios_formatados = [
            (
                ...
                funcionario[1],
                funcionario[2],
                string_to_date(funcionario[3]),
                'Ativo' if funcionario[4] else 'Inativo',
                funcionario[5],
                funcionario[6]
...
def index():
    ...
    with sqlite3.connect('funcionarios.db') as conn:
        ...
        cursor.execute("INSERT INTO funcionarios (primeiro_nome, sobrenome, data_admissao, status_funcionario, id_setor, id_cargo) VALUES (?, ?, ?, ?, ?, ?)",
                       (primeiro_nome, sobrenome, data_admissao, 1, id_setor, id_cargo))
